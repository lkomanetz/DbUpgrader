...
namespace Executioner {
	...
	public class FileSystemStore : IDataStore {
		...
		public void Add(Script script) {
			LogEntry docEntry = Deserialize(script.DocumentId);

			if (docEntry == null)
				throw new FileNotFoundException($"Document Id '{script.DocumentId}' not found.");

			if (docEntry.Scripts.Any(s => s.SysId == script.SysId))
				return;

			docEntry.Scripts.Add(new LogEntry() {
				SysId = script.SysId,
				IsComplete = script.IsComplete,
			});
			docEntry.IsComplete = docEntry.Scripts.All(s => s.IsComplete);
